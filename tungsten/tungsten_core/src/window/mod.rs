...
/// Struct used by renderer to interact with the window.
#[derive(Clone)]
...
impl WindowContext {
    pub fn get_window_ptr(&self) -> Option<*mut c_void> {
        unsafe { mem::transmute(self.r.get_xlib_window()) }
    }

    pub fn get_display_ptr(&self) -> Option<*mut c_void> {
        unsafe { mem::transmute(self.r.get_xlib_display()) }
    }
}
...
impl Window {
    /// Create a window from the settings defined in the registery.
    ...
    pub fn get_context(&self) -> WindowContext {
        WindowContext { r: self.window.clone() }
    }
}
