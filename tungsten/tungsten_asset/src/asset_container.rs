...
pub struct Container<T> {
    ...
    // Identifier used to determin if data was changed in the mean
    // time without checking the whole asset.
    changed: usize,
}
...
impl<T> Container<T> {
    pub fn new(data: T) -> Self {
        ...
        d.data.store(Some(Owned::new(data)), Ordering::Release);
        Container {
            data: Arc::new(d),
            changed: 0,
        }
    ...
    pub fn change(&self, data: T) {
        ...
        self.data.data.store(Some(Owned::new(data)), Ordering::Release);
